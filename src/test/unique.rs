...
fn multi_key() {
    ...
    #[cfg(feature = "expr")]
    cmp(&["unique", "{desc + 1},{seq.length}"], &fa, sel!(0, 2));
}
...
fn large() {
    ...
    cmp(&["unique", "id"], &all_fasta, &unique_fasta_inorder);
    cmp(&["unique", "num(id)"], &all_fasta, &unique_fasta_inorder);
...
#[test]
fn map_out() {
    with_tmpdir("st_unique_mapout_", |td| {
        let fa = ">s1 a=1\nSEQ\n>s2 a=1\nSEQ\n>s3 a=2\nSEQ\n>s4 a=1\nSEQ\n";
        ...
        let out = td.path("map.tsv");

        cmp(&["unique", "attr(a)", "--map-out", &out], fa, unique_fa);
        ...
        assert_eq!(&out.content(), long_star);
        cmp(
            &["unique", "attr(a)", "--map-out", &out, "--map-fmt", "wide"],
            fa,
            unique_fa,
        );
        assert_eq!(&out.content(), wide);
        cmp(
            &[
                "unique",
                "attr(a)",
        ...
        assert_eq!(&out.content(), wide_comma);
        cmp(
            &[
                "unique",
                "attr(a)",
        ...
        assert_eq!(&out.content(), wide_key);
    });
}
