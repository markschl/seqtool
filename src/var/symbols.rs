...
enum Value {
    ...
    Text(
        Vec<u8>,
        LazyValue<Result<Option<i64>, ()>>,
        LazyValue<Result<Option<f64>, ()>>,
    ),
...
impl Table {
    ...
    pub fn get_int(&self, id: usize) -> CliResult<Option<i64>> {
        match *self.get(id) {
            Value::Int(i, _) => Ok(Some(i)),
            Value::Float(f, _) => Ok(Some(f as i64)),
            Value::Text(ref s, ref v, _) => get_num(s, v).map_err(|_| {
                format!(
    ...
    pub fn get_float(&self, id: usize) -> CliResult<Option<f64>> {
        match *self.get(id) {
            Value::Float(f, _) => Ok(Some(f)),
            Value::Int(i, _) => Ok(Some(i as f64)),
            Value::Text(ref s, _, ref v) => get_num(s, v).map_err(|_| {
...
#[inline]
fn get_num<T: Copy + FromStr>(
    s: &[u8],
    value: &LazyValue<Result<Option<T>, ()>>,
) -> Result<Option<T>, ()> {
